...
async function verificarDivergenciasPetrolina() {
  try {
    console.log('🔍 Verificando divergências de estoque na filial Petrolina...\n')

    // Divergências de estoque em Petrolina
    ...
    if (divergenciasEstoque.rows.length > 0) {
      console.log(`⚠️  Encontradas ${divergenciasEstoque.rows.length} divergências de ESTOQUE em Petrolina (top 20):\n`)
      ...
      console.log('✅ Nenhuma divergência de ESTOQUE encontrada em Petrolina!')
    }

    // Estatísticas gerais de Petrolina
    console.log('\n\n📊 Estatísticas gerais de Petrolina:\n')
...
verificarDivergenciasPetrolina()
