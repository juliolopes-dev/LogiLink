...
async function verificarDivergenciasBonfim() {
  try {
    console.log('🔍 Verificando divergências de estoque na filial Bonfim...\n')

    // Divergências de estoque em Bonfim
    ...
    if (divergenciasEstoque.rows.length > 0) {
      console.log(`⚠️  Encontradas ${divergenciasEstoque.rows.length} divergências de ESTOQUE em Bonfim (top 20):\n`)
      ...
      console.log('✅ Nenhuma divergência de ESTOQUE encontrada em Bonfim!')
    }

    // Estatísticas gerais de Bonfim
    console.log('\n\n📊 Estatísticas gerais de Bonfim:\n')
